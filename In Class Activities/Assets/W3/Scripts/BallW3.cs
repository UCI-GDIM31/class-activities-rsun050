...
public class BallW3 : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        _rigidbody.linearVelocity *= _speedMultiplier; // higher speedmultiplier makes ball move faster
    ...
    private float GetColorMultiplier(float spd_x, float spd_y)
    {
        float spd_avg = (spd_x + spd_y) / 2;
        if(spd_avg > _speedThreshold)
		{
            return 1.5f;
		} else
		{
