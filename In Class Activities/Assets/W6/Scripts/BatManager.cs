...
public class BatManager : MonoBehaviour
{
    ...
    private void Update()
    {
		...
		{
            _newTextTimers.SetValue(_newTextTimers.ElementAt(i) + Time.deltaTime, i);
		}
        ...
        {
            BatW6 bat = _bats.ElementAt(i);
            float distance_to_player = Vector3.Distance(bat.transform.position, _playerTransform.position);

            if (distance_to_player < _interactDistance)
            {
                bat.ChasePlayer();
                CreateReactions(bat);
            }
    ...
    private void CreateReactions(BatW6 bat)
    {
        ...
        string message = _messages.ElementAt(UnityEngine.Random.Range(0, _messages.Length - 1));
